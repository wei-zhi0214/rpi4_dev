...
#include "motor_uapi.h"   // 你 motor driver / app 共用的 UAPI
#include "enc_uapi.h"     // 你 encoder driver / app 共用的 UAPI
...
int main(int argc, char **argv)
{
    ...
    int u_step             = atoi(argv[3]);     // e.g. 200..600
    int hold_ms            = atoi(argv[4]);     // e.g. 4000
    int sample_ms          = atoi(argv[5]);     // e.g. 10
    ...
    // helper lambda: set motor both sides same
    ...
    for (;;) {
        ...
        // set PWM according to phase
        int u = 0;
        if (t_ms >= pre_ms && t_ms < pre_ms + hold_ms) u = u_step;
        set_u(u);

        // read encoder snapshot
        if (read(fe, &s_now, sizeof(s_now)) != sizeof(s_now)) {
            fprintf(stderr, "read encoder failed: %s\n", strerror(errno));
            break;
        }

        // compute delta count & dt
        int32_t dcount = s_now.count - s_prev.count;
        double dt_ms = (double)(t_ns - last_ns) / 1e6;
        double dt_s  = (t_ns - last_ns) / 1e9;

        // rpm from delta count
        double rev_s = 0.0;
        double rpm = 0.0;
        if (dt_s > 0.0) {
            rev_s = ((double)dcount) / (COUNTS_PER_REV * dt_s);
            rpm = 60.0 * rev_s;
        }
        ...
        // sleep to sample period
        usleep(sample_ms * 1000);
    }

    // stop
    xioctl(fm, MOTOR_IOC_STOP, NULL);
    en = 0;
